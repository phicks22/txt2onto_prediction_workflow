...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-index_col",
        help="Column name storing index IDs in the GT labels",
        default="index",
        type=str,
    )
    parser.add_argument(
        "-group_col",
        help="Column name storing group IDs in the GT labels",
        default="group",
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-outdir", 
        help="outdir to calculated data", 
        required=True, 
        type=str
    )
    ...
    best_th_df = pl.read_csv(args.best_threshold, separator="\t")

    prior = [None] * len(best_th_df["task"])
    num_pos = [None] * len(best_th_df["task"])
    num_pred_pos = [None] * len(best_th_df["task"])
    ...
    for file in tqdm(
        ...
        if task in best_th_df["task"].to_list():
            ...
            columns_to_select = [] if args.is_study else [args.group_col]
            selected_columns = columns_to_select + [args.index_col, task]
            label_data_sub = (
            ...
            pred_label_data = pred_data.select(["ID", "prob"]).join(
                label_data_sub, left_on="ID", right_on=args.index_col, how="right"
            )
            selected_columns = columns_to_select + [args.index_col, task, "prob"]
            ...
            annotation_sub = annotation_sub.rename({"ID": args.index_col})
            ...
            index = best_th_df.with_row_index().filter(pl.col("task") == task).select("index").item(0, 0)
    ...
    combined_data.write_csv(args.outdir)
